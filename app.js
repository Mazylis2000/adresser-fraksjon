...
window.addEventListener("load", async () => {
  ...
  // ---------- Nearby markers: grouped by same lat/lon (single point with multiple days) ----------
  ...
  async function plotNearbyAddressesGrouped(rows, maxMarkers = 5) {
    ...
    // take limited rows, but group by exact address string first
    const subset = rows.slice(0, Math.min(rows.length, 150)); // allow some grouping
    const byAddr = new Map(); // addr -> { days:Set, sampleRow }
    for (const r of subset) {
      const addr = `${r.gate} ${r.husnumr||""}, ${r.postnumr} ${r.sted}`.replace(/\s+/g," ").trim();
      if (!byAddr.has(addr)) byAddr.set(addr, { days: new Set(), row: r });
  ...
  async function findDaysForAreaFromDB(input){
    ...
    if (prefix3.length < 3) return { ok:false, message:"Postkode må ha minst 3 siffer (prefix)." };

    const codes = FRAKSJON_CODES[input.fraksjonGroup];
    if (!codes || codes.size === 0) return { ok:false, message:"Ingen fraksjon-koder satt i FRAKSJON_CODES." };

    // Query: prefix3 + avfall codes
    ...
    const wideRows = data || [];
    const wideDays = Array.from(new Set(wideRows.map(r=>r.ukedag).filter(d=>d>=1&&d<=7))).sort((a,b)=>a-b);

    return { ok:true, prefix3, wideRows, wideDays, count: wideRows.length };
  }

  function renderResult(text){ setText("resultText", text); }
  ...
  function buildRowsFromSheet(sheet){
    ...
    for (const r of rows) {
      const postnumr = cleanDigits(pickField(r, ["Postnumr","Postnummer","Postnr","Postkode"]));
      const gate = pickField(r, ["Gate/vei","Gate","Vei","Adresse","Gatevei"]);
      const hus = String(pickField(r, ["Husnumr","Husnummer","Husnr"]) ?? "").trim();
  ...
  async function adminImportExcel(file){
    ...
    // POST to Vercel API with auth token
  ...
  el("adminFile").addEventListener("change", async (e) => {
    ...
    try {
      ...
      setText("adminMsg", `OK. Upserted: ${res.upserted} rows. Chunks: ${res.chunks}.`);
    } catch (err) {
